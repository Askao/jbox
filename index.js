...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        ws.gender = radishMsg.gender;
        ws.bodyweight = radishMsg.body_weight;
        ws.hoursdrinking = radishMsg.hours_drinking;
        ws.spiritlevel = radishMsg.spirit_level;
        console.log("body: " + ws.bodyweight);
        switch (radishMsg.messageType) {
            case 'CREATE_ROOM_REQUEST': {
                let id = makeid(4);
                while (rooms.has(id)) {
                    // Create random id's until a new one is found
                    id = makeid(4);
                }
            ...
            case 'ROOM_JOIN_REQUEST': {
                if (!rooms.has(ws.room)) {
                    // Tried to connect to a non-existent roomFRF
                    ...
                    const response = {
                        ...
                        nickname: ws.nick,
                        gender: ws.gender,
                        body_weight: ws.bodyweight,
                        hours_drinking: ws.hoursdrinking,
                        spirit_level: ws.spiritlevel
                    }
                    ws.send(JSON.stringify(response));
                } else {
                    ...
                    if (findPlayerInRoom(ws.room, ws.nick) !== undefined) {
                        ...
                        if(ws.bodyweight > 1000){
                            const response = {
                                messageType: 'INVALID_WEIGHT_INPUT',
                                roomCode: ws.room,
                                nickname: ws.nick,
                                gender: ws.gender,
                                body_weight: ws.bodyweight,
                            ...
                            if(ws.hoursdrinking < 0 || ws.hoursdrinking > 24 || ws.hoursdrinking === ""){
                                const response = {
                                    messageType: 'INVALID_HOURS_DRINKING_INPUT',
                                    roomCode: ws.room,
                                    nickname: ws.nick,
                                    gender: ws.gender,
                                    body_weight: ws.bodyweight,
                                ...
                                if(ws.spiritlevel < 0 || ws.spiritlevel > 100 || ws.spiritlevel === ""){
                                    const response = {
                                        messageType: 'INVALID_SPIRIT_INPUT',
                                        roomCode: ws.room,
                                        nickname: ws.nick,
                                        gender: ws.gender,
                                        body_weight: ws.bodyweight,
